...
# ChatGPT
# 2024.06 (June).11
# Below is a Linux Bash shell script that renames all files with the extension `*.MTS.AVI` to `*.AVI` in the current directory.
...
# - `for file in *.MTS.AVI; do`: Loops through all files with the `.MTS.AVI` extension.
...
# - `newfile="${file/.MTS.AVI/.AVI}"`: Removes the `.MTS` part of the extension using parameter substitution.
...
# This will rename all files with the `.MTS.AVI` extension to `.AVI` in the current directory.
