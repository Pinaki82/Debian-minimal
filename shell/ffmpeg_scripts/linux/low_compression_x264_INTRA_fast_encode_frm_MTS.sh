...
codecoption=libx264
videoquality=0
constantratefactor=20 #The range of the CRF scale is 0–51, where 0 is lossless, 23 is the default, and 51 is worst quality possible. A lower value generally leads to higher quality, and a subjectively sane range is 17–28.
#
# NOTE: Keep the value between 10 to 22.
# NOTE: Intel media encoders and Intel drivers must be found on the system.
#
...
for inputfile in "$inputfolder"/*.* ; do
    ...
    ffmpeg -i "$inputfile" -c:v $codecoption -g 1 -sc_threshold 1G -flags +cgop -movflags "faststart" \
    -q:v $videoquality -preset $SPEED -crf $constantratefactor \
    -filter:v "yadif=0:-1:1, scale=trunc(iw/2)*2:trunc(ih/2)*2" \
