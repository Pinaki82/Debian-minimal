...
# De-interlace option can be disabled by deleting the line that contains:
...
for inputfile in "$inputfolder"/*.* ; do
    ...
    -vtag $videotag -g $gop \
    -c:a $audiocodec -q:a $audioquality -ar $audiosamplerate -ab $audiobitrate -ac $audiochannel \
